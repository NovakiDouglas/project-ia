...
async def predict_endpoint_plug(
    request: Request, content: PredictionRequest, version: str = None
):
    require_api_key(request)
    try:
        prediction, carbon_footprint, used_version = predict_plug(
            content.instances, version
        )
        return {
            "success": True,
...
async def predict_endpoint_lamp(
    request: Request, content: LampPredictionRequest, version: str = None
):
    require_api_key(request)
    try:
